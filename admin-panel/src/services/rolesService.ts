...
const listPermissions = async (): Promise<Permission[]> => {
  return api.get<Permission[]>('/admin/roles/permissions');
};

const listRoles = async (): Promise<RoleWithPermissions[]> => {
  return api.get<RoleWithPermissions[]>('/admin/roles');
};

const createRole = async (name: string, description?: string): Promise<Role> => {
  return api.post<Role>('/admin/roles', { name, description });
};
...
const deleteRole = async (roleId: string): Promise<void> => {
  await api.del(`/admin/roles/${roleId}`);
};
...
const removePermission = async (roleId: string, permissionId: string): Promise<void> => {
  await api.del(`/admin/roles/${roleId}/permissions/${permissionId}`);
};

const listRoleUsers = async (roleId: string): Promise<UserRoleLink[]> => {
  return api.get<UserRoleLink[]>(`/admin/roles/${roleId}/users`);
};

const assignUsersToRole = async (roleId: string, userIds: string[]): Promise<void> => {
  await api.post(`/admin/roles/${roleId}/users`, { role_ids: userIds });
};

const removeUserFromRole = async (roleId: string, userId: string): Promise<void> => {
  await api.del(`/admin/roles/${roleId}/users/${userId}`);
};

const getMyRoles = async (): Promise<Role[]> => {
  return api.get<Role[]>('/admin/roles/me');
};

const getUserRoles = async (userId: string): Promise<Role[]> => {
  return api.get<Role[]>(`/admin/roles/users/${userId}/roles`);
};
