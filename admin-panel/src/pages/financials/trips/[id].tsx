...
export default function TripFinancialDetailPage() {
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-3">
                  ...
                  <p className="text-xs text-blue-600 dark:text-blue-500">{t('fin.colBookings')}</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3">
                  <p className="text-xs text-slate-500 font-medium">{t('fin.totalGrossCard')}</p>
                  <p className="text-lg font-bold text-slate-700 dark:text-slate-300">{fmtSAR(detail.total_gross)}</p>
                  <p className="text-xs text-slate-400">{t('fin.platformCutRow', { amount: fmtSAR(detail.total_platform_cut) })}</p>
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-slate-800">
                      ...
                      <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-400">{t('fin.colUser')}</th>
                      ...
                      <th className="px-4 py-3 text-right font-semibold text-slate-600 dark:text-slate-400">{t('fin.platformCut')}</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {bookings.length === 0 && (
                      <tr>
                        <td colSpan={8} className="px-4 py-10 text-center text-slate-400">{t('fin.noBookingsFilter')}</td>
                      </tr>
                    )}
                    {bookings.map(b => {
                      ...
                      return (
                        <tr key={b.registration_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                          ...
                          <td className="px-4 py-3">
                            <div className="text-slate-900 dark:text-white text-sm">{b.user_name ?? '—'}</div>
                            {b.user_email && (
