...
const RequestStatusPage = () => {
  ...
  useEffect(() => {
    const fetchRequestStatus = async () => {
      try {
        ...
        setError(err.response?.data?.detail || t('requestStatus.fetchError'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const fileStatusMap: Record<string, { cls: string; label: string }> = {
    accepted: { cls: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400', label: t('requestStatus.fileAccepted') },
    rejected: { cls: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400', label: t('requestStatus.fileRejected') },
    processing: { cls: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400', label: t('requestStatus.fileUnderReview') },
  };

  const formatDt = (d: string) => new Date(d).toLocaleDateString(i18n.language === 'ar' ? 'ar-SA' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });

  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{t('requestStatus.title')}</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{t('requestStatus.subtitle')}</p>
      </div>
      ...
      <div className={`flex items-center gap-3 px-5 py-4 rounded-2xl border ${
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400">
          {request.status.toLowerCase() === 'pending' && t('requestStatus.pendingMessage')}
          {request.status.toLowerCase() === 'approved' && t('requestStatus.approvedMessage')}
          {request.status.toLowerCase() === 'denied' && (request.denial_reason ? t('requestStatus.reason', { reason: request.denial_reason }) : t('requestStatus.deniedMessage'))}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
          <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-4">{t('requestStatus.companyInfo')}</h3>
          <dl className="space-y-3">
            {[
              { label: t('requestStatus.companyName'), value: request.company_name },
              { label: t('requestStatus.companyEmail'), value: request.company_email },
              { label: t('requestStatus.companyPhone'), value: request.company_phone },
            ].map(({ label, value }) => (
              <div key={label}>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
          <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-4">{t('requestStatus.requestDetails')}</h3>
          <dl className="space-y-3">
            <div>
              <dt className="text-xs text-slate-400 dark:text-slate-500">{t('requestStatus.requestId')}</dt>
              <dd className="text-xs font-mono text-slate-700 dark:text-slate-300 mt-0.5 break-all">{request.id}</dd>
            </div>
            <div>
              <dt className="text-xs text-slate-400 dark:text-slate-500">{t('requestStatus.submitted')}</dt>
              <dd className="text-sm font-medium text-slate-900 dark:text-white mt-0.5">{request.created_at ? formatDt(request.created_at) : t('requestStatus.notAvailable')}</dd>
            </div>
            <div>
              <dt className="text-xs text-slate-400 dark:text-slate-500">{t('requestStatus.lastUpdated')}</dt>
              <dd className="text-sm font-medium text-slate-900 dark:text-white mt-0.5">{request.updated_at ? formatDt(request.updated_at) : t('requestStatus.notAvailable')}</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800">
          <h3 className="text-base font-semibold text-slate-900 dark:text-white">{t('requestStatus.uploadedDocuments')}</h3>
        </div>
        <div className="p-5">
          {uploadedFiles.length === 0 ? (
            <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-4">{t('requestStatus.noDocuments')}</p>
          ) : (
                  ...
                  <div key={file.id} className="flex items-center justify-between gap-4 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500">{(file.file_size_bytes / 1024 / 1024).toFixed(2)} MB · {file.uploaded_at ? new Date(file.uploaded_at).toLocaleDateString(i18n.language === 'ar' ? 'ar-SA' : 'en-US') : t('requestStatus.notAvailable')}</p>
                      </div>
                    </div>
