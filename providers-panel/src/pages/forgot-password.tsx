...
const ForgotPassword: React.FC = () => {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOtpSent(true);
      setStep('otp');
      setMessage({ type: 'success', text: t('forgotPassword.otpSentMsg') });
    } catch (error: any) {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      setMessage({ type: 'error', text: t('forgotPassword.passwordMismatch') });
      return;
    }

    if (newPassword.length < 8) {
      setMessage({ type: 'error', text: t('forgotPassword.passwordTooShort') });
      return;
    }
    ...
    try {
      ...
      setStep('success');
      setMessage({ type: 'success', text: t('forgotPassword.resetSuccessMsg') });
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: t('forgotPassword.otpResentMsg') });
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            {step === 'email' && t('forgotPassword.stepEmailSubtitle')}
            {step === 'otp' && t('forgotPassword.stepOtpSubtitle', { email })}
            {step === 'success' && t('forgotPassword.stepSuccessSubtitle')}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm space-y-5">
          ...
          {step === 'email' && (
            <form onSubmit={handleSendOTP} className="space-y-4">
              <div>
                <label htmlFor="email" className={labelCls}>{t('forgotPassword.emailLabel')}</label>
          ...
          {step === 'otp' && (
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div>
                <label htmlFor="otp" className={labelCls}>{t('forgotPassword.otpLabel')}</label>
              ...
              <div>
                <label htmlFor="newPassword" className={labelCls}>{t('forgotPassword.newPassword')}</label>
                <input id="newPassword" type="password" required minLength={8} value={newPassword}
                  onChange={e => setNewPassword(e.target.value)} className={inputCls} placeholder={t('forgotPassword.newPasswordPlaceholder')} />
              </div>
              <div>
                <label htmlFor="confirmPassword" className={labelCls}>{t('forgotPassword.confirmPassword')}</label>
                <input id="confirmPassword" type="password" required minLength={8} value={confirmPassword}
                  onChange={e => setConfirmPassword(e.target.value)} className={inputCls} placeholder={t('forgotPassword.confirmPasswordPlaceholder')} />
              </div>
              <button type="submit" disabled={loading}
                className="w-full py-2.5 bg-sky-500 hover:bg-sky-600 disabled:opacity-60 text-white text-sm font-semibold rounded-xl transition-colors">
          ...
          {step === 'success' && (
            <div className="text-center py-4">
              ...
              <p className="text-slate-600 dark:text-slate-400 text-sm">{t('forgotPassword.redirecting')}</p>
            </div>
          )}
        </div>
      </div>
